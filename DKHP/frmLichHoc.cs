...
namespace DKHP
{
    public partial class frmLichHoc : Form
    {
        ...
        {
            ClearPN();
            List<eLichHoc_LopHocPhan> lstLichLT = new LichHocBLL().GetLichHocLyThuyet_SinhVien(eSV.ID_SinhVien, int.Parse(cbHocKiSearch.Text.Trim()), int.Parse(cbNamHocSearch.SelectedValue.ToString().Trim()));
            List<eLichHoc_NhomThucHanh> lstLichTH = new LichHocBLL().GetLichHocThucHanh_SinhVien(eSV.ID_SinhVien, int.Parse(cbHocKiSearch.Text.Trim()), int.Parse(cbNamHocSearch.SelectedValue.ToString().Trim())); ;
